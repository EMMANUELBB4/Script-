...
function library:CreateWindow(name, version, icon)
	...
	local InsaneGay = Instance.new("ScreenGui")
	...
	ProtectInstance(InsaneGay)
	ProtectInstance(Window)
-- Assuming InsaneGay is already defined and parented somewhere above this code

-- Create a button to toggle InsaneGay visibility
local ToggleHideButton = Instance.new("TextButton")
ToggleHideButton.Size = UDim2.new(0.1, 0.1, 0.1)
ToggleHideButton.Position = UDim2.new(0, 10, 0, 10)
ToggleHideButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
ToggleHideButton.TextColor3 = Color3.fromRGB(255, 255, 255)
ToggleHideButton.Text = "Hide"
ToggleHideButton.Parent = InsaneGay

-- Function to toggle InsaneGay visibility
local function ToggleVisibility()
     
    if Window.Size == UDim2.new(0, 0, 0, 0) then
        Window.Size = UDim2.new(0, 400,0, 266.67)
        ToggleHideButton.Text = "Hide"
...
ToggleHideButton.MouseButton1Click:Connect(ToggleVisibility)


	InsaneGay.Name = RandomString
	InsaneGay.Parent = cloneref(game:GetService("CoreGui"))
	InsaneGay.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

	Window.Name = "Window"
	Window.Parent = InsaneGay
	...
	Close.Position = UDim2.new(0.933333378, 0, 0.0666666627, 0)
	...
	Minimize.Position = UDim2.new(0.923, -24, -0.2, 6)
	...
	Close.MouseButton1Click:Connect(function()
		TS:Create(Window, TweenInfo.new(0.5), {Size = UDim2.new(0, 400, 0, 0)}):Play()
		repeat
			wait()
		until Window.Size == UDim2.new(0, 400, 0, 0)
		wait(0.1)
	...
	Minimize.MouseButton1Click:Connect(function()
		if not MinimizeGui then
			MinimizeGui = true
			if Window.Size == UDim2.new(0, 400,0, 266.67) then
				TS:Create(Window, TweenInfo.new(0.25), {Size = UDim2.new(0, 400,0, 32)}):Play()
				repeat wait() until Window.Size == UDim2.new(0, 400,0, 32)
			...
			MinimizeGui = false
			if Window.Size == UDim2.new(0, 400,0, 32) then
				if Page and Page.Visible == true then
					Page.Visible = false
				end
				if Tabs and Tabs.Visible == true then
				...
				TS:Create(Window, TweenInfo.new(0.25), {Size = UDim2.new(0, 400,0, 266.67)}):Play()
			end
		end
	end)
...
-- Another Dragify function (dragify2)
function dragify2(Frame)
	...
	local function updateInput2(input)
		...
		local Position2 = UDim2.new(
			startPos2.X.Scale, startPos2.X.Offset + Delta2.X,
			startPos2.Y.Scale, startPos2.Y.Offset + Delta2.Y
		)
		TS:Create(Frame, TweenInfo.new(0.25), {Position = Position2}):Play()
	end
	...
	dragify(Window)
	TS:Create(Window, TweenInfo.new(0.5), {Size = UDim2.new(0, 400, 0, 0)}):Play()
	repeat wait() until Window.Size == UDim2.new(0, 400, 0, 0)
	wait(0.1)
	TS:Create(Window, TweenInfo.new(0.5), {Size = UDim2.new(0, 400, 0, 266.67)}):Play()
dragify2(ToggleHideButton)
	--end
	local tabs = {}
	...
	function tabs:CreateTab(name)
		...
		function mytabbuttons:CreateFrame(name)
			...
			function CreateNotification(Name, Description, Callback)
				...
				Notification2.Parent = InsaneGay
